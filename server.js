...
async function main() {
  ...
  const telegram = new TelegramClient({
    ...
    chatId: config.telegramPhotoChatId || config.telegramChatId,
    settingsFile: config.telegramSettingsFile,
    preferConfiguredChat: true,
    confirmationText: '✅ AKFIX: группа выбрана для фотографий и результатов проверки склада.',
  });
  await telegram.init();
  const telegramExpeditor = new TelegramClient({
    token: config.telegramExpeditorBotToken,
    chatId: config.telegramExpeditorChatId,
    settingsFile: config.telegramExpeditorSettingsFile,
    confirmationText: '✅ AKFIX: группа выбрана для экспедиторских расписок водителей.',
  });
  ...
  const shippingSheet = new ShippingSheetClient({
    spreadsheetId: config.shippingSpreadsheetId,
    sheetName: config.shippingSheetName,
  });
  ...
  http
    .createServer((req, res) => {
      res.setHeader(
        ...
        "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; connect-src 'self'; media-src 'self' blob:; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'"
      );
    ...
    .listen(config.port, '0.0.0.0', () => console.warn(`ProverkaSklad listening on 0.0.0.0:${config.port}`));
}
