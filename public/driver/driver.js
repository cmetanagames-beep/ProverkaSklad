...
function renderDetail({ order, bitrix, completed }) {
  const item = bitrix?.item || bitrix || {};
  const needsPhoto = /^тк(?:\s|$)/i.test(order.delivery);
  const fields = Object.entries(item).filter(([,value]) => value !== null && value !== '' && !Array.isArray(value)).slice(0, 24);
  $('#detail').innerHTML = `<div class="hero"><h1>${esc(order.client || 'Заказ')}</h1><p>Заказ № ${esc(order.orderNumber || '—')}</p></div><div class="panel facts"><div class="fact"><small>Дата</small><b>${esc(order.date || '—')}</b></div><div class="fact"><small>Склад</small><b>${esc(order.warehouse || '—')}</b></div><div class="fact"><small>Доставка</small><b>${esc(order.delivery || '—')}</b></div><div class="fact"><small>Документы</small><b>${esc(order.documents || '—')}</b></div><div class="fact"><small>Честный знак</small><b>${esc(order.marking || '—')}</b></div><div class="fact"><small>Перебивка</small><b>${esc(order.relabel || '—')}</b></div></div><div class="panel"><h2>Данные из Битрикс24</h2><div class="bitrix">${fields.length ? fields.map(([key,value]) => `<div><span>${esc(key)}</span>${esc(value)}</div>`).join('') : '<p class="note">Данные сделки не были сохранены.</p>'}</div></div>${completed ? `<div class="panel done-mark">${completed.queued ? '◷ Сохранено на телефоне. Отправится после появления интернета.' : '✓ Рейс завершён ' + new Date(completed.completedAt).toLocaleString('ru-RU')}</div>` : `<form id="completeForm" class="panel"><h2>${needsPhoto ? 'Фото экспедиторской обязательно' : 'Завершение рейса'}</h2>${needsPhoto ? '<label id="upload" class="upload"><span>📷 Сфотографировать экспедиторскую</span><input id="photo" type="file" accept="image/*" capture="environment" required></label>' : '<p class="note">Подтвердите, что груз отправлен.</p>'}<button id="complete" class="primary" ${needsPhoto ? 'disabled' : ''}>Груз отправлен</button></form>`}`;
  if (!completed) {
    const photo = $('#photo');
