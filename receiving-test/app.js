...
async function loadFile(file){
 ...
 try{
  products=await parseXlsx(file);shipmentName=file.name.replace(/\.xlsx?$/i,'');storageKey=`akfix_receiving_${file.name}_${file.size}_${file.lastModified}`;
  const saved=JSON.parse(localStorage.getItem(storageKey)||'{}');state.rows=products.map((p,i)=>saved.rows?.[i]||{done:false,alloc:[{qty:p.plan,date:'',cell:'',comment:''}]});state.open=0;state.query='';state.complete=false;render();toast(`Загружено позиций: ${products.length}`);
...
function render(){
 ...
 app.innerHTML=`<div class="shell"><header class="top"><button class="back" id="changeFile">‹</button><h1>Приёмка</h1><span></span></header><section class="summary"><h2>${esc(shipmentName)}</h2><p>${done} из ${products.length} позиций · данные из Excel</p><div class="progress"><div class="track"><div class="bar" style="width:${done/products.length*100}%"></div></div><b>${Math.round(done/products.length*100)}%</b></div></section><input class="search" placeholder="Поиск по артикулу или названию" value="${esc(state.query)}"><div class="labels"><span>Артикул / товар</span><span>План</span><span>Статус</span></div><section>${filtered.map(p=>{const r=state.rows[p.i],open=state.open===p.i;return`<article class="item ${open?'open':''} ${r.done?'done':''}"><div class="item-head" data-open="${p.i}"><span class="num">${p.i+1}</span><span class="product"><b>${esc(p.article)}</b><small>${esc(p.name)}<br>${esc(p.pack)}</small></span><span class="qty">${p.plan.toLocaleString('ru-RU')}</span><span class="status">${r.done?'Готово':'Ожидает'} ${open?'⌃':'⌄'}</span></div>${open?`<div class="form">${r.alloc.map((a,j)=>allocation(a,j,r.alloc.length)).join('')}<div class="actions"><button class="btn outline" id="split">Разделить размещение</button><button class="btn primary" id="save">Сохранить позицию</button></div></div>`:''}</article>`}).join('')||'<div class="empty">Ничего не найдено</div>'}</section><footer class="finish"><button class="btn primary" id="finish" ${done<products.length?'disabled':''}>Завершить приёмку</button></footer></div>`;wire();
}
...
function wire(){
 ...
 const save=document.querySelector('#save');if(save)save.onclick=()=>{const row=state.rows[state.open],sum=row.alloc.reduce((s,a)=>s+Number(a.qty||0),0);if(row.alloc.some(a=>!a.qty||!a.date||!a.cell))return toast('Заполните количество, срок годности и ячейку');if(sum!==products[state.open].plan)return toast(`Распределено ${sum}, по плану ${products[state.open].plan}`);row.done=true;persist();state.open=Math.min(state.open+1,products.length-1);toast('Позиция сохранена');render()};
